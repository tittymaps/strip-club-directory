...
export default async function CityPage({ params }: { params: { state: string, city: string } }) {
  ...
  const bikiniCafeCount = clubs.filter(isBikiniCafe).length
  const lingerieCount = clubs.filter(isLingerieStudio).length
  const stripClubs = clubs.filter(c => !isBikiniCafe(c) && !isLingerieStudio(c))
  ...
  const faqSchema = clubs.length > 0 ? {
    ...
    "mainEntity": [
      ...
      },
      {
        ...
        "acceptedAnswer": {
          "@type": "Answer",
          "text": `${cityName} has ${fullNudeCount} full nude club${fullNudeCount === 1 ? '' : 's'}, ${toplessCount} topless club${toplessCount === 1 ? '' : 's'}${bikiniCount > 0 ? `, and ${bikiniCount} bikini club${bikiniCount === 1 ? '' : 's'}` : ''}${bikiniCafeCount > 0 ? `, plus ${bikiniCafeCount} bikini coffee shop${bikiniCafeCount === 1 ? '' : 's'}` : ''}${lingerieCount > 0 ? `, and ${lingerieCount} lingerie modeling studio${lingerieCount === 1 ? '' : 's'}` : ''}. Full nude clubs typically do not serve alcohol due to local regulations, while topless clubs often have a full bar.`
        }
      },
      {
        "@type": "Question",
    ...
    <div style={{ background: '#0D0F1E', minHeight: '100vh', color: 'white', fontFamily: 'sans-serif', paddingBottom: 80 }}>
      ...
      <div style={{ padding: '16px 16px 8px' }}>
        ...
        <p style={{ color: '#8890c0', fontSize: 13, margin: 0 }}>
          ...
          {dancers.length > 0 ? ` · ${dancers.length} featured dancer${dancers.length === 1 ? '' : 's'}` : ''}
        </p>
      </div>
        ...
        <div style={{ padding: '8px 16px 0' }}>
          <div style={{ background: '#131629', borderRadius: 12, border: '1px solid #1e2140', padding: 16 }}>
            <p style={{ color: '#ccc', fontSize: 14, lineHeight: 1.6, margin: '0 0 14px' }}>
              ...
              {' '}in {cityName}, {stateName}. Updated {MONTH_YEAR}.
            </p>
          ...
          <ol style={{ color: '#ccc', fontSize: 13, lineHeight: 1.8, paddingLeft: 20, margin: 0 }}>
            {topClubs.map(club => (
              <li key={club.id}>
                ...
                {isBikiniCafe(club) ? ' (bikini coffee shop)' : isLingerieStudio(club) ? ' (lingerie modeling studio)' : ''}
              </li>
            ))}
          </ol>
        </div>
