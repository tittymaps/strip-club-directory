...
export async function POST(request: Request) {
  ...
  const profileUrl = `https://tittymaps.com/dancers/${dancer_id}`
  ...
  const html = `
    <div style="font-family: sans-serif; max-width: 500px; margin: 0 auto; padding: 24px; background: #0D0F1E; color: white; border-radius: 12px;">
      ...
      ${is_featured ? featuredHtml : standardHtml}
      <a href="${profileUrl}" style="display: inline-block; background: #FF2D78; color: white; padding: 12px 24px; border-radius: 8px; text-decoration: none; font-weight: bold; margin-bottom: 16px;">View your profile</a>
